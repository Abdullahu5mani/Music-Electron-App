import { E as w, c as f, b as S, r as h, t as T, S as E, m as I, v as $, B as z, w as D, T as N } from "./main-D7ryyLE7.js";
    ...
    for (; this.tokenizer.position < t; ) {
      ...
      if (n)
        switch (u.startNext(n)) {
          case c.ReadNext:
            if (i.id, g(`Read element: name=${x(n)}{id=0x${i.id.toString(16)}, container=${!!n.container}} at position=${o}`), n.container) {
            ...
            break;
          case c.SkipElement:
            g(`Go to next element: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${o}`);
            break;
          case c.IgnoreElement:
            g(`Ignore element: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${o}`), await this.tokenizer.ignore(i.len);
            break;
          case c.SkipSiblings:
            g(`Ignore remaining container, at: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${o}`), await this.tokenizer.ignore(t - this.tokenizer.position);
            break;
          case c.TerminateParsing:
            return g(`Terminate parsing at element: name=${x(n)}, element.id=0x${i.id}, container=${!!n.container} at position=${o}`), s;
        }
      else
        switch (i.id) {
          case 236:
            ...
            break;
          default:
            g(`parseEbml: parent=${x(a)}, unknown child: id=${i.id.toString(16)} at position=${o}`), this.padding += i.len, await this.tokenizer.ignore(i.len);
        }
    }
    return s;
  }
        ...
        switch (u.id) {
          ...
          case 524531317:
            if (this.flagUseIndexToSkipClusters && this.seekHead) {
              const s = this.seekHead.seek.find((i) => i.position + this.seekHeadOffset > this.tokenizer.position);
              if (s) {
          ...
          case 357149030:
            {
              ...
              if (typeof o.duration == "number") {
                ...
                await this.addTag("segment:title", o.title), this.metadata.setFormat("duration", Number(r));
              }
            }
            break;
          case 374648427:
            {
              ...
              if (o?.entries) {
                ...
                const n = o.entries.filter((r) => r.trackType === N.audio).reduce((r, m) => !r || m.flagDefault && !r.flagDefault || m.trackNumber < r.trackNumber ? m : r, null);
          ...
          case 307544935:
            {
              ...
              await Promise.all(o.tag.map(async (n) => {
                const r = n.target, m = r?.targetTypeValue ? D[r.targetTypeValue] : r?.targetType ? r.targetType : "track";
