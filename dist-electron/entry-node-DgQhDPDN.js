import { D as Debug, g as getAugmentedNamespace, a as getDefaultExportFromCjs } from "./main-BKOuB9HX.js";
import { randomFillSync, randomUUID } from "node:crypto";
const debug$1 = Debug("musicbrainz-api-node");
...
class HttpClient {
  ...
  async _fetch(method, path, options) {
    ...
    while (retryLimit > 0) {
      ...
      if (response.status === 429 || response.status === 503) {
        debug$1(`Received status=${response.status}, assume reached rate limit, retry in ${retryTimeout} ms`);
        retryLimit--;
        if (retryLimit > 0) {
          await this._delay(retryTimeout);
          continue;
  ...
  _buildUrl(path, query) {
    let url = path.startsWith("/") ? `${this.httpOptions.baseUrl}${path}` : `${this.httpOptions.baseUrl}/${path}`;
    if (query) {
...
function requireJsontoxml() {
  ...
  (function(module) {
    ...
    var process_to_xml = function(node_data, options) {
      ...
      return (function fn(node_data2, node_descriptor, level) {
        ...
        switch (type) {
          ...
          case "object":
            if (node_descriptor == 1 && node_data2.name) {
              var content = [], attributes = [];
              if (node_data2.attrs) {
                if (typeof node_data2.attrs != "object") {
                  attributes.push(" ");
                  attributes.push(node_data2.attrs);
                } else {
                  for (var key in node_data2.attrs) {
                    var value = node_data2.attrs[key];
                    attributes.push(" ");
                    attributes.push(key);
                    attributes.push('="');
              ...
              if (typeof node_data2.value != "undefined") {
                var c = "" + node_data2.value;
                content.push(options.escape && !node_data2.noescape ? esc(c) : c);
              } else if (typeof node_data2.text != "undefined") {
                var c = "" + node_data2.text;
                content.push(options.escape && !node_data2.noescape ? esc(c) : c);
              }
              if (node_data2.children) {
                content.push(fn(node_data2.children, 0, level + 1));
              }
              return makeNode(node_data2.name, content.join(""), attributes.join(""), level, !!node_data2.children);
            } else {
              var nodes = [];
    ...
    module.exports = function(obj, options) {
      var Buf = typeof Buffer !== "undefined" ? Buffer : function Buffer2() {
      };
...
function requireGetPublicSuffix() {
  ...
  getPublicSuffix$1.getPublicSuffix = getPublicSuffix2;
...
class HttpClientNode extends HttpClient {
  constructor(options) {
    super(options);
    this.cookieJar = new cookieExports.CookieJar();
  }
  registerCookies(response) {
    const cookie2 = response.headers.get("set-cookie");
    if (cookie2) {
      return this.cookieJar.setCookie(cookie2, response.url);
    }
    return Promise.resolve(void 0);
  }
  getCookies() {
...
function requireSparkMd5() {
  if (hasRequiredSparkMd5) return sparkMd5$1.exports;
...
class DigestAuth {
  ...
  digest(method, path, authHeader) {
    ...
    const digestResponse = qop ? md5(`${ha1}:${challenge.nonce}:${nc}:${cnonce}:${qop}:${ha2}`) : md5(`${ha1}:${challenge.nonce}:${ha2}`);
    const authValues = {
...
class RateLimitThreshold {
  ...
  async limit() {
    ...
    this.queue.push((/* @__PURE__ */ new Date()).getTime());
    return delay;
  }
}
var LinkType;
(function(LinkType2) {
  LinkType2[LinkType2["license"] = 302] = "license";
  LinkType2[LinkType2["production"] = 256] = "production";
  LinkType2[LinkType2["samples_IMDb_entry"] = 258] = "samples_IMDb_entry";
  LinkType2[LinkType2["get_the_music"] = 257] = "get_the_music";
  LinkType2[LinkType2["purchase_for_download"] = 254] = "purchase_for_download";
  LinkType2[LinkType2["download_for_free"] = 255] = "download_for_free";
  LinkType2[LinkType2["stream_for_free"] = 268] = "stream_for_free";
  LinkType2[LinkType2["crowdfunding_page"] = 905] = "crowdfunding_page";
  LinkType2[LinkType2["other_databases"] = 306] = "other_databases";
  LinkType2[LinkType2["Allmusic"] = 285] = "Allmusic";
})(LinkType || (LinkType = {}));
const debug = Debug("musicbrainz-api");
let MusicBrainzApi$1 = class MusicBrainzApi {
  ...
  async post(entity, xmlMetadata) {
    ...
    do {
      ...
      if (response.statusCode === StatusCodes.UNAUTHORIZED) {
        ...
        const relPath = response.requestUrl.pathname;
  ...
  async addUrlToRecording(recording, url2add, editNote = "") {
    const formData = {};
    formData["edit-recording.name"] = recording.title;
...
class MusicBrainzApi2 extends MusicBrainzApi$1 {
  initHttpClient() {
  ...
  async login() {
    ...
    const success = response.status === StatusCodes.MOVED_TEMPORARILY && response.headers.get("location") === redirectUri;
    if (success) {
...
export {
  HttpClient,
  LinkType,
  MusicBrainzApi2 as MusicBrainzApi
};
