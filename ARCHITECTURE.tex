...
\begin{tikzpicture}[scale=0.7]
    % Title
    \node[font=\bfseries] at (6,4.5) {Batch Processing 3 Songs with Rate Limiting};
    
    % Time axis
    \draw[thick, ->] (0,0) -- (13,0) node[right] {Time (seconds)};
    
    % Time markers
    \foreach \x in {0,1,...,12} {
        \draw (\x,0.1) -- (\x,-0.1);
        \node[below, font=\tiny] at (\x,-0.2) {\x s};
    }
    
    % Song 1
    \node[left, font=\small\bfseries] at (0,3.5) {Song 1:};
    \fill[blue!40] (0,3.3) rectangle (0.02,3.7) node[right, font=\tiny, midway] { Fingerprint (19ms)};
    \fill[orange!40] (0.5,3.3) rectangle (1,3.7) node[midway, font=\tiny] {AcoustID};
    \fill[green!40] (1.6,3.3) rectangle (2.7,3.7) node[midway, font=\tiny] {MusicBrainz};
    \fill[purple!40] (3.8,3.3) rectangle (4.9,3.7) node[midway, font=\tiny] {Cover Art};
    \node[right, font=\tiny, green] at (5,3.5) {✓ Done (5s)};
    
    % Song 2
    \node[left, font=\small\bfseries] at (0,2.5) {Song 2:};
    \fill[blue!40] (0.5,2.3) rectangle (0.52,2.7) node[right, font=\tiny, midway] { Fingerprint};
    \fill[orange!40] (1,2.3) rectangle (1.5,2.7) node[midway, font=\tiny] {AcoustID};
    \fill[green!40] (2.7,2.3) rectangle (3.8,2.7) node[midway, font=\tiny] {MusicBrainz};
    \fill[purple!40] (4.9,2.3) rectangle (6,2.7) node[midway, font=\tiny] {Cover Art};
    \node[right, font=\tiny, green] at (6.2,2.5) {✓ Done (6.5s)};
    
    % Song 3
    \node[left, font=\small\bfseries] at (0,1.5) {Song 3:};
    \fill[blue!40] (1,1.3) rectangle (1.02,1.7) node[right, font=\tiny, midway] { Fingerprint};
    \fill[orange!40] (1.5,1.3) rectangle (2,1.7) node[midway, font=\tiny] {AcoustID};
    \fill[green!40] (3.8,1.3) rectangle (4.9,1.7) node[midway, font=\tiny] {MusicBrainz};
    \fill[purple!40] (6,1.3) rectangle (7.1,1.7) node[midway, font=\tiny] {Cover Art};
    \node[right, font=\tiny, green] at (7.3,1.5) {✓ Done (8s)};
    
    % Delay arrows
    \draw[<->, thick, red, dotted] (1,3.1) -- (1.6,3.1) node[midway, below, font=\tiny] {500ms};
    \draw[<->, thick,red, dotted] (2.7,3.1) -- (3.8,3.1) node[midway, below, font=\tiny] {1100ms};
    
    % Legend
    \node[draw, fill=blue!40, minimum width=0.4cm, minimum height=0.2cm] at (9.5,3.8) {};
    \node[right, font=\tiny] at (9.8,3.8) {Fingerprint (19ms)};
    
    \node[draw, fill=orange!40, minimum width=0.4cm, minimum height=0.2cm] at (9.5,3.4) {};
    \node[right, font=\tiny] at (9.8,3.4) {AcoustID (500ms delay)};
    
    \node[draw, fill=green!40, minimum width=0.4cm, minimum height=0.2cm] at (9.5,3.0) {};
    \node[right, font=\tiny] at (9.8,3.0) {MusicBrainz (1100ms delay)};
    
    \node[draw, fill=purple!40, minimum width=0.4cm, minimum height=0.2cm] at (9.5,2.6) {};
    \node[right, font=\tiny] at (9.8,2.6) {Cover Art (1100ms delay)};
    
    % Summary box
    \node[draw=gray, fill=gray!5, thick, text width=2.5cm, align=left, font=\tiny] at (11,1.5) {
        \textbf{Why Stagger?}\\[0.05cm]
        • Prevents ban\\
        • Respects APIs\\
        • ~2.7s per song\\
        • 3 songs = 8s total
...
\end{tikzpicture}
\caption{Rate Limiting in Action: Batch Processing 3 Songs}
\label{fig:rate_limit_timeline}
\end{figure}

\begin{tipbox}
\textbf{Real-World Analogy: Coffee Shop Drive-Through}

Imagine a drive-through coffee shop with signs:
\begin{itemize}
    \item "Maximum 2 cars per minute" (AcoustID: 500ms delay)
    \item "Maximum 1 car per minute" (MusicBrainz: 1100ms delay)  
\end{itemize}

You can't just flood the line! You must wait between customers. Same with APIs - we respect their speed limits to avoid getting banned!
\end{tipbox}
...
\begin{tipbox}
\textbf{Real-World Analogy: Music Library Card Catalog}

Imagine a physical music library with:
\begin{itemize}
    \item \textbf{metadata\_cache.db} - A file cabinet tracking which CDs you've cataloged (prevents re-scanning)
    \item \textbf{playlists.db} - A notebook where you write custom playlists referencing CD locations
\end{itemize}

The "CASCADE DELETE" is like: if you tear out a playlist page, all songs listed on that page disappear too!
\end{tipbox}

\section{Complete Database Schema Visualization}

\begin{figure}[ht!]
\centering
\begin{tikzpicture}[
    node distance=1.2cm and 2.5cm,
    table/.style={rectangle split, rectangle split parts=3, draw=blue!70, fill=blue!10, thick, minimum width=4cm, text width=3.8cm, font=\small},
    relationship/.style={draw=green!60, thick, ->, >=stealth},
]
    % Metadata Cache DB
    \node[table] (cache) {
        \textbf{metadata\_cache}
        \nodepart{second}
        \underline{filePath} TEXT\\
        fileHash TEXT\\
        scannedAt INTEGER\\
        mbid TEXT\\
        hasMetadata INTEGER
    ...
    % Playlists table
    \node[table, right=3cm of cache] (playlists) {
        \textbf{playlists}
        \nodepart{second}
        ...
        updatedAt INTEGER
        \nodepart{third}
        \textit{PK: id (auto-inc)}
    };
    
    % Playlist songs junction table
    \node[table, below=1.5cm of playlists] (songs) {
        \textbf{playlist\_songs}
        \nodepart{second}
        \underline{playlistId} INTEGER\\
        \underline{filePath} TEXT\\
        position INTEGER\\
        addedAt INTEGER
        \nodepart{third}
        \textit{PK: (playlistId, filePath)}\\
        \textit{FK: playlistId → playlists(id)}
    };
    
    % Relationships
    \draw[relationship] (playlists) -- (songs) node[midway, right, font=\tiny] {1:N\\CASCADE\\DELETE};
    
    % Database labels
    \node[above=0.3cm of cache, font=\bfseries, text=blue] {metadata-cache.db};
    \node[above=0.3cm of playlists, font=\bfseries, text=blue] {playlists.db};
    
    % Info boxes
    \node[draw=orange, fill=orange!5, thick, text width=3.5cm, align=left, font=\tiny, below=0.5cm of cache] (info1) {
        \textbf{Purpose:}\\
        Tracks which files have been scanned to avoid re-processing\\[0.1cm]
        \textbf{Hash Formula:}\\
        SHA256(path + filesize + mtime)\\[0.1cm]
        \textbf{Performance:}\\
        1000+ lookups in <10ms
    };
    
    \node[draw=orange, fill=orange!5, thick, text width=3.5cm, align=left, font=\tiny, below=0.5cm of songs] (info2) {
        \textbf{Purpose:}\\
        Store user-created playlists\\[0.1cm]
        \textbf{Junction Table:}\\
        playlist\_songs links playlists to files (many-to-many)\\[0.1cm]
        \textbf{Index:}\\
        idx\_playlist\_songs\_playlist for fast queries
    };
\end{tikzpicture}
\caption{Complete Database Schema: Entity-Relationship Diagram}
\label{fig:database_schema}
\end{figure}

\begin{notebox}
\textbf{Why Two Separate Databases?} \\
\textbf{metadata-cache.db} is for system use (fast lookups during scanning).\\
\textbf{playlists.db} is for user data (create/delete playlists frequently).\\
Separation prevents locking issues and allows independent backups!
\end{notebox}
...
\begin{tipbox}
\textbf{Real-World Analogy: Recording Studio Audio Engineer}

Imagine you're an audio engineer trying to transcribe song lyrics:
\begin{enumerate}
    \item \textbf{Isolate Vocals} - Remove instruments using special filters (like studio mixing)
    \item \textbf{Optimize for Speech} - Convert to format AI understands (16kHz mono WAV)
    \item \textbf{AI Transcription} - Feed to speech recognition system (Whisper)
    \item \textbf{Clean Up} - Delete temporary files (keep studio clean)
\end{enumerate}
This is exactly what the AI Lyrics pipeline does!
\end{tipbox}

\section{Complete AI Lyrics Pipeline}

\begin{figure}[ht!]
\centering
\begin{tikzpicture}[
    node distance=0.7cm,
    inputbox/.style={rectangle, draw=blue!70, fill=blue!15, thick, minimum width=3.8cm, minimum height=0.7cm, text width=3.6cm, align=center, font=\small},
    ffmpegbox/.style={rectangle, draw=orange!70, fill=orange!15, thick, minimum width=3.8cm, minimum height=0.7cm, text width=3.6cm, align=center, font=\small},
    whisperbox/.style={rectangle, draw=purple!70, fill=purple!15, thick, minimum width=3.8cm, minimum height=0.7cm, text width=3.6cm, align=center, font=\small},
    outputbox/.style={rectangle, draw=green!70, fill=green!15, thick, minimum width=3.8cm, minimum height=0.7cm, text width=3.6cm, align=center, font=\small},
    progressbox/.style={rectangle, draw=cyan!70, fill=cyan!10, thick, minimum width=2.8cm, minimum height=0.6cm, text width=2.6cm, align=center, font=\tiny},
]
    % Input
    \node[inputbox] (input) at (0,0) {1. Input Audio\\song.mp3\\(stereo, any sample rate)};
    
    % Progress 1
    \node[progressbox, right=1cm of input] (prog1) {Progress Event:\\``Isolating vocals...''\\(20\%)};
    
    % FFmpeg Stage 1
    \node[ffmpegbox, below=of input] (ffmpeg1) {2. FFmpeg: Extract Vocals\\8-Stage DSP Pipeline\\Remove instruments};
    
    % Temp file 1
    \node[inputbox, fill=gray!10, below=of ffmpeg1] (temp1) {temp\_vocals.wav\\(16kHz mono WAV)};
    
    % Progress 2
    \node[progressbox, right=1cm of temp1] (prog2) {Progress Event:\\``Preparing for AI...''\\(40\%)};
    
    % Check Whisper
    \node[ffmpegbox, below=of temp1] (check) {3. Check Whisper.cpp\\Auto-download if missing};
    
    % Whisper transcription
    \node[whisperbox, below=of check] (whisper) {4. Whisper AI\\Speech-to-Text\\(selected model)};
    
    % Progress 3
    \node[progressbox, right=1cm of whisper] (prog3) {Progress Event:\\``Transcribing...''\\(60\%)};
    
    % Output
    \node[outputbox, below=of whisper] (output) {5. Lyrics Text\\``Verse 1: ...\\nChorus: ...''};
    
    % Progress 4
    \node[progressbox, right=1cm of output] (prog4) {Progress Event:\\``Complete!''\\(100\%)};
    
    % Cleanup
    \node[ffmpegbox, fill=red!10, below=of output] (cleanup) {6. Cleanup\\Delete temp\_vocals.wav};
    
    % Final
    \node[outputbox, fill=green!20, below=of cleanup] (done) {7. Display in UI\\LyricsPanel shows result};
    
    % Arrows
    \draw[->, thick] (input) -- (ffmpeg1);
    \draw[->, thick] (ffmpeg1) -- (temp1);
    \draw[->, thick] (temp1) -- (check);
    \draw[->, thick] (check) -- (whisper);
    \draw[->, thick] (whisper) -- (output);
    \draw[->, thick] (output) -- (cleanup);
    \draw[->, thick] (cleanup) -- (done);
    
    % Progress arrows
    \draw[->, thick, dotted, cyan] (input) -- (prog1);
    \draw[->, thick, dotted, cyan] (temp1) -- (prog2);
    \draw[->, thick, dotted, cyan] (whisper) -- (prog3);
    \draw[->, thick, dotted, cyan] (output) -- (prog4);
    
    % Info box
    \node[draw=gray, fill=gray!5, thick, minimum width=3.5cm, minimum height=6.5cm, text width=3.3cm, align=left, font=\tiny, left=1.5cm of whisper] (info) {
        \textbf{FFmpeg 8-Stage DSP:}\\[0.05cm]
        1. High-pass filter (80Hz)\\
        2. Low-pass filter (3kHz)\\
        3. Stereo to mono mix\\
        4. Phase inversion\\
        5. Freq. reduction\\
        6. Mid-range boost\\
        7. Compression\\
        8. Normalization\\[0.1cm]
        \textbf{Why 16kHz Mono?}\\
        • Whisper trained on 16kHz\\
        • Mono = smaller file\\
        • Faster processing\\[0.1cm]
        \textbf{Temp File Cleanup:}\\
        Prevents disk bloat\\
        (WAV files are huge!)\\[0.1cm]
        \textbf{Total Time:}\\
        • tiny.en: ~30s\\
        • base.en: ~1min\\
        • medium.en: ~3min\\
...
\end{tikzpicture}
\caption{Complete AI Lyrics Generation Pipeline (8-Stage FFmpeg + Whisper)}
\label{fig:ai_lyrics_pipeline}
\end{figure}

\begin{notebox}
\textbf{Why Vocal Isolation?} Music contains instruments that confuse speech recognition AI. By removing instruments with FFmpeg filters, we give Whisper only the vocals, dramatically improving accuracy!
\end{notebox}
...
\subsection{Complete Playback Control Flow}

\begin{figure}[ht!]
\centering
\begin{tikzpicture}[
    node distance=0.7cm and 1cm,
    statebox/.style={rectangle, draw=blue!70, fill=blue!15, thick, minimum width=3cm, minimum height=0.7cm, text width=2.8cm, align=center, font=\small},
    actionbox/.style={diamond, draw=orange!70, fill=orange!15, thick, minimum width=2.2cm, minimum height=1cm, text width=1.8cm, align=center, font=\tiny\bfseries},
    resultbox/.style={rectangle, draw=green!70, fill=green!15, thick, minimum width=3cm, minimum height=0.7cm, text width=2.8cm, align=center, font=\small},
]
    % Initial state
    \node[statebox] (init) at (0,0) {IDLE\\No song playing};
    
    % User clicks song
    \node[actionbox, below=1cm of init] (click) {User\\Clicks\\Song};
    
    % Loading
    \node[statebox, below=1cm of click] (load) {LOADING\\Create Howl object};
    
    % Playing
    \node[resultbox, below=of load] (playing) {PLAYING\\Audio streaming};
    
    % Pause button
    \node[actionbox, left=2cm of playing] (pause) {User\\Clicks\\Pause};
    
    % Paused state
    \node[statebox, fill=yellow!15, below=of pause] (paused) {PAUSED\\Audio stopped};
    
    % Resume
    \node[actionbox, right=2cm of paused] (resume) {User\\Clicks\\Play};
    
    % Next button
    \node[actionbox, right=2.5cm of playing] (next) {User\\Clicks\\Next};
    
    % Check queue
    \node[actionbox, below=1cm of next] (checkQ) {Songs\\Left in\\Queue?};
    
    % Next song
    \node[resultbox, left=1cm of checkQ] (nextSong) {Load Next Song\\(or random if shuffle)};
    
    % End of queue
    \node[statebox, fill=red!15, right=1cm of checkQ] (end) {END\\Back to idle};
    
    % Volume/Seek
    \node[actionbox, below=1.5cm of playing] (volseek) {Volume\\or Seek};
    
    \node[resultbox, below=of volseek] (adjust) {Adjust Playback\\Howl.volume()\\Howl.seek()};
    
    % Main flow arrows
    \draw[->, thick] (init) -- (click);
    \draw[->, thick] (click) -- (load);
    \draw[->, thick] (load) -- (playing);
    \draw[->, thick] (playing) -- (pause);
    \draw[->, thick] (pause) -- (paused);
    \draw[->, thick] (paused) -- (resume);
    \draw[->, thick, dashed] (resume) |- (playing);
    \draw[->, thick] (playing) -- (next);
    \draw[->, thick] (next) -- (checkQ);
    \draw[->, thick] (checkQ) -- node[above, font=\tiny] {Yes} (nextSong);
    \draw[->, thick] (checkQ) -- node[above, font=\tiny] {No} (end);
    \draw[->, thick, dashed] (nextSong) |- (load);
    \draw[->, thick] (playing) -- (volseek);
    \draw[->, thick] (volseek) -- (adjust);
    \draw[->, thick, dashed] (adjust) -| (playing);
    
    % Labels
    \node[draw=gray, fill=gray!5, thick, text width=2.5cm, align=left, font=\tiny, right=1.5cm of end] (info) {
        \textbf{Howler.js API:}\\[0.05cm]
        • new Howl(src)\\
        • .play()\\
        • .pause()\\
        • .stop()\\
...
\end{tikzpicture}
\caption{Complete Audio Player User Interaction Flow}
\label{fig:audio_player_flow}
\end{figure}

\begin{notebox}
\textbf{Why Howler.js?} It handles all the complex audio stuff: format detection (MP3/FLAC/WAV), cross-browser compatibility, streaming, and provides a simple API. We just call \inlinecode{.play()} and it works everywhere!
\end{notebox}
