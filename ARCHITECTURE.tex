...
% Suppress overfull/underfull hbox warnings in log
\usepackage{silence}
...
\begin{figure}[ht!]
    ...
    \begin{tikzpicture}[
        ...
        % Nodes
        \node (main) [process] {\textbf{Main Process} \\ \small (Node.js, File System, OS)};
        \node (bridge) [bridge, right=of main] {\textbf{IPC Bridge} \\ \small (Preload Script)};
        \node (renderer) [process, right=of bridge] {\textbf{Renderer Process} \\ \small (React UI, Web Audio API)};
        ...
        % Labels
        \node [above=0.2cm of main] {\color{sectioncolor}\textbf{Backend}};
        \node [above=0.2cm of renderer] {\color{sectioncolor}\textbf{Frontend}};
        \node [below=0.5cm of bridge, text width=6cm, align=center] {\small \textit{Communication via window.electronAPI}};
    \end{tikzpicture}
    \caption{Process Isolation and IPC Communication Architecture}
...
\begin{tikzpicture}[
    ...
    impl/.style={draw, rounded corners, minimum width=3cm, minimum height=1.2cm, align=center},
    arrow/.style={->, thick, >=stealth}
]
    % Interface
    ...
    % Implementations
    \node[impl, fill=green!15] (music) at (-3, -0.5) {MusicPlayer\\implements Playable};
    \node[impl, fill=blue!15] (video) at (0, -0.5) {VideoPlayer\\implements Playable};
    \node[impl, fill=purple!15] (podcast) at (3, -0.5) {PodcastPlayer\\implements Playable};
...
export class ParallelMetadataScanner {
    ...
    constructor(concurrency?: number) {
        this.numWorkers = concurrency ?? Math.max(2, 16)
    }
...
\begin{tikzpicture}[
    node distance=0.3cm,
    hub/.style={draw, circle, fill=yellow!20, minimum size=1.5cm, font=\small\bfseries},
...
\begin{tikzpicture}[
    ...
    api/.style={draw, ellipse, fill=yellow!10, minimum width=2.5cm},
    arrow/.style={->, thick, >=stealth}
]
    \node[box, fill=blue!10] (fp) {Generate Fingerprint\\(\techterm{fpcalc})};
    \node[api, below=of fp] (acoustid) {AcoustID API};
    \node[api, right=2.5cm of acoustid] (mb) {MusicBrainz API};
    \node[api, below=of mb] (caa) {Cover Art Archive};
    \node[box, left=2.5cm of caa, fill=green!10] (taglib) {Embed Tags\\(taglib-wasm)};

    \draw[arrow] (fp) -- (acoustid);
    \draw[arrow] (acoustid) -- node[above, font=\scriptsize] {MBID} (mb);
    \draw[arrow] (mb) -- node[right, font=\scriptsize] {Release ID} (caa);
    \draw[arrow] (caa) -- (taglib);
    \draw[arrow] (mb) |- (taglib);
\end{tikzpicture}
\caption{Song Identification API Flow}
\label{fig:api_flow}
\end{figure}
...
\inlinecode{downloadYouTubeAudio(options)} & \inlinecode{Promise<\{success, filePath?, error?, title?\}>} & Downloads audio from YouTube \\
\inlinecode{getVideoTitle(url, ytDlp)} & \inlinecode{Promise<string>} & Fetches video title before download \\
\inlinecode{checkBinaryVersion(path, onProgress?)} & \inlinecode{Promise<\{isUpToDate, versions\}>} & Checks if yt-dlp is up to date \\
\inlinecode{downloadBinaryWithProgress(path, onProgress?)} & \inlinecode{Promise<void>} & Downloads yt-dlp binary \\
...
\inlinecode{generateFingerprintsParallel(paths, onProgress?)} & \inlinecode{Promise<FingerprintJobResult[]>} & Parallel fingerprinting \\
...
\inlinecode{generateFingerprintsBatch(paths, onProgress?)} & \inlinecode{Promise<BatchFingerprintResult[]>} & Parallel batch \\
...
\centering
\begin{tikzpicture}[
    step/.style={draw, rounded corners, minimum width=3cm, minimum height=0.7cm, align=center, font=\small},
    arrow/.style={->, thick, >=stealth}
]
    \node[step, fill=blue!10] (song) at (0, 0) {Audio File};
    \node[step, fill=purple!10] (ffmpeg) at (3.5, 0) {FFmpeg};
    \node[step, fill=yellow!15] (wav) at (7, 0) {16kHz WAV};
    \node[step, fill=green!10] (whisper) at (10.5, 0) {Whisper.cpp};
    \node[step, fill=orange!15] (lyrics) at (14, 0) {Lyrics Text};
...
\midrule
musicHandlers & 6 channels & Scan folders, read/write metadata \\
fingerprintHandlers & 4 channels & Generate audio fingerprints \\
apiHandlers & 4 channels & External API calls (AcoustID, MusicBrainz) \\
cacheHandlers & 7 channels & Track scan status in SQLite \\
lyricsHandlers & 3 channels & AI lyrics transcription \\
youtubeHandlers & 5 channels & YouTube download, binary management \\
playlistHandlers & 7 channels & CRUD operations for playlists \\
watchHandlers & 3 channels & File system watching \\
systemHandlers & 5 channels & Platform info, settings \\
\midrule
\textbf{Total} & \textbf{44 channels} & \\
\bottomrule
\end{tabular}
\caption{Complete IPC Channel Count by Module}
\end{table}
